...
The uniform direction is `u = m0/‖m0‖`. `H5_geom` samples in its orthogonal
...
### Matching the budget properly

`H5_geom` is generated by drawing the same isotropic `Δm` as `H5_box` and then
projecting out the uniform component, `Δm ← Δm − (Δm·u)u`. **Projection removes
budget**, so sampling both arms at the same width would silently hand `H5_box`
a 1.5× larger perturbation and make either outcome uninterpretable — a win for
`H5_geom` could not be separated from noise, and a loss could be entirely the
smaller budget.

Measured over 4×10⁵ draws, before any policy was trained:

| width `w` | `E‖Δm‖` box | `E‖Δm‖` geom | ratio |
|---:|---:|---:|---:|
| 0.10 | 0.01189 | 0.00804 | 0.676 |
| 0.20 | 0.02375 | 0.01610 | 0.678 |
| 0.40 | 0.04751 | 0.03215 | 0.677 |

So `H5_geom`'s width is scaled by **1.478**, which restores the match to three
decimal places (`w = 0.10` box → `E‖Δm‖` 0.01188; `w = 0.1479` geom → 0.01191).
The projection is verified exact in the same check: `E|Δm·u|` is 6×10⁻¹⁹ for
`H5_geom` against 7.6×10⁻³ for `H5_box`.

The runner asserts the realised `E‖Δm‖` of the two arms agree to within 2%
